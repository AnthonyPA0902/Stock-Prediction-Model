...
val_metrics = trainer.validate(tft, val_loader, verbose=False)
orig_val_loss = val_metrics[0]["val_loss"]
print(f"[ORIGINAL MODEL] val_loss: {orig_val_loss:.6f}")

predictions = tft.predict(
    val_loader, mode="raw", return_x=True, trainer_kwargs=dict(accelerator="cpu")
)
# predictions_vs_actual = tft.calculate_prediction_actual_by_variable(
